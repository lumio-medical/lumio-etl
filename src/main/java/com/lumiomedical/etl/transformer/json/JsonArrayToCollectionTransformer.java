 ...
 */
public class JsonArrayToCollectionTransformer<T, C extends Collection<T>> implements Transformer<ArrayNode, C>
{
    private final Function<JsonNode, T> mapper;
    private final Collector<T, ?, C> collector;

    public JsonArrayToCollectionTransformer(Function<JsonNode, T> mapper, Collector<T, ?, C> collector)
    {
        this.mapper = mapper;
